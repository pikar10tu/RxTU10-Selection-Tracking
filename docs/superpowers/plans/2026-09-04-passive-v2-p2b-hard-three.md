...
```js
🔴 **ไวรัสในเทสสองเคสนี้ต้องเป็นเพ็ทที่ลงทะเบียนจริง ไม่ใช่ `__blank__`** — โค้ดอ่านเพดานจาก
`passiveFor(inf.from)` ⇒ ถ้าไวรัสไม่มีพาสสีฟในทะเบียน จะหา `vpart` ไม่เจอแล้วเพดานตกไปที่ค่าสำรอง
ทำให้เทสไม่ได้ทดสอบเพดานจริง · ให้ลงทะเบียน `PET_PASSIVES.__virus` แบบเดียวกับงานย่อย 5/6
แล้วลบใน `finally` (บทเรียนจากงานย่อย 6 ที่เจอปัญหาเดียวกัน)
⚠️ **ห้ามแก้ด้วยการเติมค่าคงที่เพดานลงโค้ดจริง** — เพดานเป็นเลขปรับได้ที่อยู่ใน `value.max` และ P4 เป็นเจ้าของ
ค่าคงที่ในโค้ดคือแหล่งความจริงที่สองที่จะเพี้ยนเงียบๆ
